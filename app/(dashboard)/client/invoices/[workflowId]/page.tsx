...
export default async function ClientInvoiceDetailPage({ params }: { params: Promise<{ workflowId: string }> }) {
  ...
  if (!workflow || workflow.clientUserId !== principal.id) redirect("/access-blocked");
  const canPay = workflow.financial.balanceDue > 0 && ["approved", "issued", "partially_paid", "overdue", "etims_accepted"].includes(workflow.financial.invoiceStatus);
