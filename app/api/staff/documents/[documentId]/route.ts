...
export async function GET(_request: Request, { params }: { params: Promise<{ documentId: string }> }) {
  ...
  try {
    ...
    const object = await getR2Client().send(new GetObjectCommand({ Bucket: configuration.bucketName, Key: record.storageKey }));
    if (!object.Body) return new Response("Not found", { status: 404 });
    const bytes = await object.Body.transformToByteArray();
    const filename = record.name.replace(/["\r\n]/g, "-");
    return new Response(Buffer.from(bytes), { headers: { "Cache-Control": "private, no-store", "Content-Disposition": `attachment; filename="${filename}"`, "Content-Type": record.contentType, "X-Content-Type-Options": "nosniff" } });
  } catch {
