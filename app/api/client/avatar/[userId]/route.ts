...
import { getClientAvatarRecord } from "@/repositories/client-profile-repository";
...
export async function GET(
  ...
  if (principal.id !== userId && !principal.permissions.includes("clients.read")) {
    return new Response("Forbidden", { status: 403 });
  }

  const avatar = await getClientAvatarRecord(userId);
