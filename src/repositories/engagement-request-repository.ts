...
export async function sendEngagementQuotation(input: {
  requestId: string;
  amount: number;
  currency: string;
  actor: Principal;
  ...
  await connectToDatabase();
  if (!Types.ObjectId.isValid(input.requestId) || !Number.isFinite(input.amount) || input.amount <= 0) return false;
  const request = await EngagementRequestModel.findOne({
    _id: input.requestId,
    requestType: "quotation",
    status: { $in: ["quotation_requested", "quotation_preparing"] },
  }).exec();
  if (!request) return false;

  request.status = "quotation_sent";
  request.quotationAmount = Math.round(input.amount * 100) / 100;
  request.quotationCurrency = input.currency.trim().toUpperCase().slice(0, 3) || "KES";
  request.reviewedAt = new Date();
  request.timeline.push({
    at: new Date(),
    title: "Quotation ready",
    detail: `${request.quotationCurrency} ${request.quotationAmount.toLocaleString("en-KE")} for the selected services.`,
    clientVisible: true,
  });
  await request.save();
  await createCommunicationNotification({
    ...
    description: `${request.reference} is ready for your review and acceptance.`,
    relatedModule: "engagements",
    relatedRecordId: request._id.toString(),
    actionUrl: "/client/engagements",
  ...
  await writeAuditLog({
    actor: input.actor,
    action: "request.quotation_sent",
    resourceType: "EngagementRequest",
    resourceId: request._id.toString(),
    newValues: { amount: request.quotationAmount, currency: request.quotationCurrency },
  });
  return true;
}

export async function acceptEngagementQuotation(requestId: string, principal: Principal) {
  await connectToDatabase();
  ...
  request.timeline.push({
    ...
    detail: "The administration team can now create the engagement workspace.",
    clientVisible: true,
  });
  await request.save();
  await writeAuditLog({
    actor: principal,
    action: "request.quotation_accepted",
    resourceType: "EngagementRequest",
    resourceId: request._id.toString(),
