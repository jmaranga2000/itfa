...
export function EngagementExecutionWorkspace({
  ...
  const continueTab: EngagementWorkspaceTab = !sectionState.overview.complete
    ? "overview"
      ...
      {activeTab === "documents" ? (
        <MobileSection title="Documents">
          ...
          <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            {editable ? (
              <Card className="h-fit shadow-none" id="document-upload">
                <CardHeader><CardTitle>Upload document</CardTitle><CardDescription>{isClient ? "Send evidence or a requested replacement securely to the engagement team." : "Add a working file for review or a final output for controlled release."}</CardDescription></CardHeader>
                <CardContent className="grid gap-4">
                  ...
                  <form action={isClient ? uploadClientDocumentAction : uploadEngagementDocumentAction} className="grid gap-3 border-t border-border pt-4" encType="multipart/form-data">
                    ...
                    <div className="grid gap-2"><Label htmlFor="workspace-document">File</Label><Input id="workspace-document" name="document" required type="file" /></div>
                    <SubmitButton pendingText="Uploading..."><Upload className="h-4 w-4" />Upload document</SubmitButton>
                  </form>
                </CardContent>
              </Card>
            ) : null}
