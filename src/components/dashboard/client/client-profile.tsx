...
const errors: Record<string, string> = {
  ...
  "avatar-missing": "Choose a profile picture before uploading.",
  "avatar-size": "The profile picture must be smaller than 3 MB.",
  "avatar-type": "Use a JPG, PNG or WebP image.",
  "avatar-upload": "The profile picture could not be stored. Please try again or contact support.",
...
function initials(profile: ClientProfileRecord) {
  return `${profile.firstName.at(0) ?? ""}${profile.lastName.at(0) ?? ""}`.toUpperCase() || "C";
}
...
export function ClientProfile({
  ...
  return (
    <AdminPageSurface
      description="Keep your contact identity, profile picture and sign-in password up to date."
      icon={UserRound}
      title="My profile"
    >
      {success ? (
      ...
      <div className="grid gap-6 p-5 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="grid justify-items-center gap-3 text-center">
            ...
            <div className="min-w-0">
              ...
              <Badge className="mt-3" tone="teal">
                {profile.role === "client_representative" ? "Client representative" : "Client"}
              </Badge>
            </div>
          </div>
          <form action={uploadClientAvatarAction} className="grid gap-3" encType="multipart/form-data">
        ...
        <div className="grid content-start gap-6">
          <form action={updateClientProfileAction} className="grid gap-4 rounded-md border border-border p-5">
            <div>
              <h2 className="font-bold text-foreground">Personal details</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Your email address is tied to your IFTA account and cannot be changed here.
              </p>
            </div>
          ...
          <form action={changeClientPasswordAction} className="grid gap-4 rounded-md border border-border p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                For your security, changing your password signs out every active session.
              </p>
            </div>
            <div className="grid gap-2">
