"use server";

import { randomUUID } from "node:crypto";
import { PutObjectCommand } from "@aws-sdk/client-s3";
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";
import { z } from "zod";
import { hashPassword, verifyPassword } from "@/features/auth/password";
import { isPasswordPolicySatisfied } from "@/features/auth/password-policy";
import { requireUser } from "@/features/auth/server";
import { revokeUserSessions } from "@/features/auth/session";
import { getR2Client, getR2Configuration } from "@/lib/r2";
import {
  getClientPasswordRecord,
  updateClientAvatarRecord,
  updateClientPasswordHash,
  updateClientProfileDetails,
} from "@/repositories/client-profile-repository";

const profileSchema = z.object({
  firstName: z.string().trim().min(2).max(80),
  lastName: z.string().trim().min(2).max(80),
});
const avatarTypes = new Set(["image/jpeg", "image/png", "image/webp"]);
const MAX_AVATAR_SIZE = 3 * 1024 * 1024;

async function requireClientAccount() {
  const principal = await requireUser();
  if (!principal.roleKeys.some((role) => role === "client" || role === "client_representative")) {
    redirect("/sign-in");
  }
  return principal;
}

export async function updateClientProfileAction(formData: FormData) {
  const principal = await requireClientAccount();
  const parsed = profileSchema.safeParse({
    firstName: formData.get("firstName"),
    lastName: formData.get("lastName"),
  });
  if (!parsed.success) redirect("/client/profile?error=profile");

  await updateClientProfileDetails(principal.id, parsed.data);
  revalidatePath("/client/profile");
  redirect("/client/profile?saved=1");
}

export async function uploadClientAvatarAction(formData: FormData) {
  const principal = await requireClientAccount();
  const avatar = formData.get("avatar");
  if (!(avatar instanceof File) || avatar.size === 0) {
    redirect("/client/profile?error=avatar-missing");
  }
  if (avatar.size > MAX_AVATAR_SIZE) redirect("/client/profile?error=avatar-size");
  if (!avatarTypes.has(avatar.type)) redirect("/client/profile?error=avatar-type");

  const extension = avatar.type === "image/png"
    ? "png"
    : avatar.type === "image/webp"
      ? "webp"
      : "jpg";
  const key = `avatars/${principal.id}/${randomUUID()}.${extension}`;

  try {
    const configuration = getR2Configuration();
    await getR2Client().send(new PutObjectCommand({
      Bucket: configuration.bucketName,
      Key: key,
      Body: Buffer.from(await avatar.arrayBuffer()),
      ContentType: avatar.type,
      CacheControl: "private, max-age=3600",
    }));
    await updateClientAvatarRecord(principal.id, {
      avatarKey: key,
      avatarContentType: avatar.type,
    });
  } catch (error) {
    console.error("Unable to upload client avatar.", error);
    redirect("/client/profile?error=avatar-upload");
  }

  revalidatePath("/client/profile");
  redirect("/client/profile?avatar=1");
}

export async function changeClientPasswordAction(formData: FormData) {
  const principal = await requireClientAccount();
  const currentPassword = String(formData.get("currentPassword") ?? "");
  const newPassword = String(formData.get("newPassword") ?? "");
  const confirmPassword = String(formData.get("confirmPassword") ?? "");
  if (newPassword !== confirmPassword || !isPasswordPolicySatisfied(newPassword)) {
    redirect("/client/profile?error=password-policy");
  }

  const user = await getClientPasswordRecord(principal.id);
  if (!user || !(await verifyPassword(currentPassword, user.passwordHash))) {
    redirect("/client/profile?error=current-password");
  }
  if (await verifyPassword(newPassword, user.passwordHash)) {
    redirect("/client/profile?error=password-same");
  }

  await updateClientPasswordHash(principal.id, await hashPassword(newPassword));
  await revokeUserSessions(principal.id);
  redirect("/sign-in?passwordChanged=1");
}
